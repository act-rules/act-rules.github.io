...
For at least one element of the set comprised by the test target and all of its [visible][] [descendant][] elements in the [flat tree][], at least one of the following is true when the element is neither [focused][] nor [hovered][]:

- **distinguishing style**: the element has a [distinguishable style][] from the **non-link line text** elements; or
