...
The rule applies to any HTML `input` element with a `type` attribute set to [`image`](<https://html.spec.whatwg.org/#image-button-state-(type=image)>), that is [included in the accessibility tree][].
