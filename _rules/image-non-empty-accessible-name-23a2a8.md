...
- `svg` elements expect an [implicit role][] of `graphics-document` per [svg-aam](https://www.w3.org/TR/svg-aam-1.0/#details-id-66). There are popular browsers that expose SVG elements with a different [semantic role][], such as `image`, `generic` or `SvgRoot`.
