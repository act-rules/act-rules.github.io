...
description: |
  This rule checks that links that are embedded in a paragraph, list or cell have a style not based on color alone that distinguishes them from the surrounding text
...
Each target element has a [distinguishing style][] not based on color alone, from the _ancestor_ element and any of its [descendant][] HTML elements that have [visible][] [text nodes][text node] and do not have a [semantic role][] of ['link'][link] or inheriting from ['link'][link].
