...
rule_type: atomic
description: |
	This rule checks that the `style` attribute is not used to prevent adjusting `word-spacing` spacing by using `!important`, except if it's at least `0.16` times the font size.
