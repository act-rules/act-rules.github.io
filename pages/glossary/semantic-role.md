...
1. **Conflict** If the element has an [explicit role][] which is either `none` or `presentation`, but the element is [included in the accessibility tree][], or would be when it is not [excluded from the accessibility tree](https://www.w3.org/TR/wai-aria-1.2/#tree_exclusion), then its _semantic role_ is its **[implicit role][]**.
