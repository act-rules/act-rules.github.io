...
$(function() {
	...
	function manageFloatingHeader(node) {
		...
		if(!floatingHeader) {
			...
			floatingHeader.setAttribute('class', 'container floating-header');
			document.body.insertBefore(floatingHeader, document.body.firstChild);
		}
	...
	function getLastOutofBoundHeader() {
		var floatingHeader = document.querySelector('header.floating-header');
		...
		var outOfBoundHeaders = Array.from(headerNodes).filter(function(header) {
			return (header.getBoundingClientRect().bottom) < floatingHeaderHeight;
		});
