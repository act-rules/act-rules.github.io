...
This rule applies to any set of any two or more HTML or SVG elements that have the [semantic role](#semantic-role) of `link`, or a role that inherits from the `link` role, that are [included in the accessibility tree](#included-in-the-accessibility-tree), have [matching](#matching-characters) [accessible names](#accessible-name) that do not only consist of [whitespace](#whitespace), and that exist within the same [document tree](https://dom.spec.whatwg.org/#document-trees) or [shadow tree](https://dom.spec.whatwg.org/#shadow-trees).
