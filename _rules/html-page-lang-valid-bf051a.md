...
**Note:** `html` elements within `iframe` and `object` elements are not applicable as `iframe` and `object` elements create [nested browsing contexts](https://html.spec.whatwg.org/#nested-browsing-context). These elements can embed any third party content. Testing the validity of the `lang` attribute on this content is left out of scope of this rule as it can be difficult to achieve. However, as these elements are meant to provide a layer of isolation, the language of the [parent browsing context](https://html.spec.whatwg.org/#parent-browsing-context) may not be inherited. Thus, embedded documents without a `lang` attribute of their own are likely to cause accessibility issues, but this should be tested separately.
