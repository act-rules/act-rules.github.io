...
This rule applies to any HTML `<audio>` `<video>` or `<source>` `src` file content `duation` of more than 3 seconds, that is set to `autoplay` and or `loop`, and has audio output that is not `paused` or `muted`.
